...
class TestCompanies:
    def setup_method(self):
        company = db.session.query(Company).filter(Company.tax_number == 'XXXXXXXXXXXXXXXXX2')
        company.delete()

    def teardown_method(self):
        if getattr(self, 'company_id', None) is not None:
            company = db.session.query(Company).filter(Company.company_id == self.company_id)
            company.delete()

    def check_company(self, data, action='create'):
    ...
    def check_ad_company(self, data, action='create'):
        assert data.get("company_id") == self.company_id
        assert data.get("company_name") == '测试专用公司2'
        assert data.get("tax_number") == 'XXXXXXXXXXXXXXXXX2'
    ...
    def test_workflow(self, client):
        ...
        data = res.get('data')
        assert data, res
        self.check_company(data, action='update')
    ...
    def test_ad_workflow(self, client):
        ...
        for path, _headers in [(path1, headers1), (path2, headers2)]:
            ...
            for company in companies:
                if company.get('company_id') == self.company_id:
                    self.check_ad_company(company, action='update')
                    break
            else:
                assert 0, data
