...
def create_sign(values, timestamp, nonce, token_or_secret=None):
    ...
    _logger.debug(f'sign str: {sign_str}')
...
def load_token(token, algorithm='HS256', verify=True):
    return jwt.decode(token, CONFIG.TOKEN_SALT, verify=verify, algorithm=algorithm)
