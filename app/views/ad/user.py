...
class Users(Resource):
    ...
    def post(self):
        ...
        user = UserModel(
            ...
            api_key=gen_api_key(kwargs['user_name'])
        )
        user.pwd = kwargs['password']
    ...
    search_parser.add_argument(
        'user_state', type=str, choices=[v.value for k, v in constants.ResourceState.__members__.items()])
    ...
    def get(self):
        ...
        if kwargs.get('user_state') is not None:
            cursor = cursor.filter(UserModel.user_state.contains(kwargs['user_state']))
