...
from app import marshalling_models
...
creation_model = Model('user_creation', {})
update_model = Model('user_update', {})
login_schema = {
    "title": "user_login",
    "type": "object",
    "description": "用户登录",
...
login_resp = user_api.clone('LoginBody', marshalling_models.base_resp, {
    'data': fields.Nested(
        user_api.model(
            'LoginData', {
...
class User(Resource):
    ...
    def post(self, uid):
        ...
        kwargs = g.args.copy()
        if uid != g.user.user_id:
            e = errors.PreconditionRequired(errmsg='User not found')
            logger.log(level=e.log_level, msg=e.log)
            return e.to_dict_response()
        if kwargs.get("password") is not None:
            g.user.pwd = kwargs.pop("password")
        try:
            g.user.update(kwargs, user=g.user)
        except errors.DBStatementError as e:
            logger.log(level=e.log_level, msg=e.log)
            return e.to_dict_response()
        return {
            'return_code': 200,
